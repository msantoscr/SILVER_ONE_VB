...
Public Class frm_material
    ...
    Sub FILL_DATA()
        Try
            Connect_Database()
            Command = New SqlCommand("SP_SILV_MATERIAL_VIEW", connection) With {.CommandType = CommandType.StoredProcedure}
            With Command
    ...
    Private Sub BTN_SAVE_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BTN_SAVE.ItemClick
        ...
        Else
            Try
                Connect_Database()
                Command = New SqlCommand("SP_SILV_MATERIAL_INSERT", connection) With {.CommandType = CommandType.StoredProcedure}
                With Command
                    .Parameters.Add("@MA_NAME_MATERIAL", SqlDbType.NVarChar, 100).Value = TXT_NAME.Text

                    If (TXT_OBSERVATIONS.Text = "") Then
                        .Parameters.AddWithValue("@MA_DESCRIPTION", DBNull.Value)
                    Else
                        .Parameters.Add("@MA_DESCRIPTION", SqlDbType.NVarChar, 200).Value = TXT_OBSERVATIONS.Text
                    End If

                    .Parameters.Add("@MA_ACTIVE_INACTIVE", SqlDbType.Int).Value = C_ACTIVE_INACTIVE.CheckState
                    .Parameters.Add("@MA_USER_CREATOR", SqlDbType.NVarChar, 100).Value = frn_main_form.LBL_USERNAME.Caption
    ...
    Private Sub BTN_EDIT_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BTN_EDIT.ItemClick
        ...
        If (VALIDA = True) Then
            ...
            Exit Sub
        Else
            If (XtraMessageBox.Show("¿DESEA MODIFICAR EL REGISTRO DEL TIPO DE MATERIAL?", "SISTEMA", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No) Then
                Exit Sub
            Else
                Try
                    Connect_Database()
                    Command = New SqlCommand("SP_SILV_MATERIAL_EDIT", connection) With {.CommandType = CommandType.StoredProcedure}
                    With Command
                        .Parameters.Add("@ID_MATERIAL", SqlDbType.Int).Value = TXT_ID.Text
                        ...
                        .Parameters.Add("@MA_ACTIVE_INACTIVE", SqlDbType.Int).Value = C_ACTIVE_INACTIVE.CheckState
                        .Parameters.Add("@MA_USER_UPDATE", SqlDbType.NVarChar, 100).Value = frn_main_form.LBL_USERNAME.Caption
    ...
    Private Sub BTN_DELETE_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BTN_DELETE.ItemClick
        ...
        If (VALIDA = True) Then
            ...
            Exit Sub
        Else
            If (XtraMessageBox.Show("¿DESEA ELIMINAR EL REGISTRO DEL TIPO DE MATERIAL?", "SISTEMA", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No) Then
                Exit Sub
            Else

                Try
                    Connect_Database()
                    Command = New SqlCommand("SP_SILV_MATERIAL_DELETE", connection) With {.CommandType = CommandType.StoredProcedure}
                    With Command
                        .Parameters.Add("@ID_MATERIAL", SqlDbType.Int).Value = TXT_ID.Text
    ...
    Private Sub frm_material_Load(sender As Object, e As EventArgs) Handles Me.Load
        FILL_DATA()
    End Sub
