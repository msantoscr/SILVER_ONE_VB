...
Public Class frm_agents
    ...
    Sub FILL_DATA()
        Try
            Connect_Database()
            Command = New SqlCommand("SP_SILV_AGENTS_VIEW", connection) With {.CommandType = CommandType.StoredProcedure}
            With Command
    ...
    Sub LIST_ROUTE()
        Try
            Connect_Database()
            Command = New SqlCommand("SP_LIST_ROUTE", connection) With {.CommandType = CommandType.StoredProcedure}
            With Command
            ...
            With CB_ROUTE
                .DataSource = DataT
                .ValueMember = "ID_ROUTE"
                .DisplayMember = "RO_NAME_ROUTE"
            End With
        Catch ex As Exception
            XtraMessageBox.Show(ex.Message, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error)
        Finally
    ...
    Sub LIST_VALUE_ROUTE()
        Try
            Connect_Database()
            Command = New SqlCommand("SP_LIST_SILV_ROUTES_AGENTS", connection) With {.CommandType = CommandType.StoredProcedure}
            With Command
                .Parameters.Add("@ID_ROUTE", SqlDbType.Int).Value = TXT_ID.Text.Trim
                Dim Message As New SqlParameter("@MENSAJE", SqlDbType.NVarChar, 200)
                Message.Direction = ParameterDirection.Output
                Command.Parameters.Add(Message)
                Rows = Command.ExecuteNonQuery
    ...
    Private Sub BTN_SAVE_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BTN_SAVE.ItemClick
        ...
        Else
            Try
                Connect_Database()
                Command = New SqlCommand("SP_SILV_AGENTS_INSERT", connection) With {.CommandType = CommandType.StoredProcedure}
                With Command
                    .Parameters.Add("@ID_ROUTE", SqlDbType.Int).Value = CB_ROUTE.SelectedValue
                    .Parameters.Add("@AG_NAME_AGENT", SqlDbType.NVarChar, 100).Value = TXT_NAME.Text
                    .Parameters.Add("@AG_COMISION", SqlDbType.NVarChar, 10).Value = TX_COM.Text
                    If (TXT_OBSERVATIONS.Text = "") Then
                        .Parameters.AddWithValue("@AG_OBSERVATIONS", DBNull.Value)
                    Else
                        .Parameters.Add("@AG_OBSERVATIONS", SqlDbType.NVarChar, 200).Value = TXT_OBSERVATIONS.Text
                    End If
                    .Parameters.Add("@AG_ACTIVE_INACTIVE", SqlDbType.Int).Value = C_ACTIVE_INACTIVE.CheckState
                    .Parameters.Add("@AG_USER_CREATOR", SqlDbType.NVarChar, 100).Value = frn_main_form.LBL_USERNAME.Caption
    ...
    Private Sub BTN_EDIT_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BTN_EDIT.ItemClick
        ...
        If (VALIDA = True) Then
            ...
            Exit Sub
        Else
            If (XtraMessageBox.Show("¿DESEA MODIFICAR EL REGISTRO DEL AGENTE?", "SISTEMA", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No) Then
                Exit Sub
            Else
                Try
                    Connect_Database()
                    Command = New SqlCommand("SP_SILV_AGENTS_EDIT", connection) With {.CommandType = CommandType.StoredProcedure}
                    With Command
                        .Parameters.Add("@ID_AGENTS", SqlDbType.Int).Value = TXT_ID.Text
                        .Parameters.Add("@ID_ROUTE", SqlDbType.Int).Value = CB_ROUTE.SelectedValue
                        .Parameters.Add("@AG_NAME_AGENT", SqlDbType.NVarChar, 100).Value = TXT_NAME.Text
                        .Parameters.Add("@AG_COMISION", SqlDbType.NVarChar, 10).Value = TX_COM.Text
                        If (TXT_OBSERVATIONS.Text = "") Then
                        ...
                        End If
                        .Parameters.Add("@AG_ACTIVE_INACTIVE", SqlDbType.Int).Value = C_ACTIVE_INACTIVE.CheckState
                        .Parameters.Add("@AG_USER_UPDATE", SqlDbType.NVarChar, 100).Value = frn_main_form.LBL_USERNAME.Caption
    ...
    Private Sub BTN_DELETE_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BTN_DELETE.ItemClick
        If (TXT_ID.Text = "") Then
            ...
            Exit Sub
        Else
            If (XtraMessageBox.Show("¿DESEA ELIMINAR EL REGISTRO DEL AGENTE?", "SISTEMA", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No) Then
                Exit Sub
            Else
                Try
                    Connect_Database()
                    Command = New SqlCommand("SP_SILV_AGENTS_DELETE", connection) With {.CommandType = CommandType.StoredProcedure}
                    With Command
                        .Parameters.Add("@ID_AGENTS", SqlDbType.Int).Value = TXT_ID.Text
    ...
    Private Sub REFRESH_ROUTE_ItemClick(sender As Object, e As ItemClickEventArgs) Handles REFRESH_ROUTE.ItemClick
        LIST_ROUTE()
    End Sub
    ...
    Private Sub G_DATA_DoubleClick(sender As Object, e As EventArgs) Handles G_DATA.DoubleClick
        Try
            ...
            TXT_NAME.Text = CType(G_DATA.GetRowCellValue(G_DATA.FocusedRowHandle, "AGENTE DE VENTAS"), String)
            ...
            If (IsDBNull(G_DATA.GetRowCellValue(G_DATA.FocusedRowHandle, "COMISION"))) Then
                TX_COM.Text = ""
            Else
                TX_COM.Text = CType(G_DATA.GetRowCellValue(G_DATA.FocusedRowHandle, "COMISION"), String)
    ...
    Private Sub frm_agents_Load(sender As Object, e As EventArgs) Handles Me.Load
        FILL_DATA()
    End Sub
End Class