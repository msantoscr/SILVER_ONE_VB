...
Public Class frm_type_doctos

    Sub FILL_DATA()
        Try
            Connect_Database()
            Command = New SqlCommand("SP_SILV_TYPE_DOCTOS_VIEW", connection) With {.CommandType = CommandType.StoredProcedure}
            With Command
    ...
    Private Sub BTN_SAVE_ItemClick(sender As Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BTN_SAVE.ItemClick
        ...
        Else
            Try
                Connect_Database()
                Command = New SqlCommand("SP_SILV_TYPE_DOCTOS_INSERT", connection) With {.CommandType = CommandType.StoredProcedure}
                With Command
                    .Parameters.Add("@TY_NAME_TYPE_DOCTO", SqlDbType.NVarChar, 100).Value = TXT_NAME.Text

                    If (TXT_OBSERVATIONS.Text = "") Then
                        .Parameters.AddWithValue("@TY_OBSERVATIONS", DBNull.Value)
                    Else
                        .Parameters.Add("@TY_OBSERVATIONS", SqlDbType.NVarChar, 200).Value = TXT_OBSERVATIONS.Text
                    End If

                    .Parameters.Add("@TY_ACTIVE_INACTIVE", SqlDbType.Int).Value = C_ACTIVE_INACTIVE.CheckState
                    .Parameters.Add("@TY_USER_CREATOR", SqlDbType.NVarChar, 100).Value = frn_main_form.LBL_USERNAME.Caption
    ...
    Private Sub BTN_EDIT_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BTN_EDIT.ItemClick
        ...
        If (VALIDA = True) Then
            ...
            Exit Sub
        Else
            If (XtraMessageBox.Show("¿DESEA MODIFICAR EL REGISTRO DEL TIPO DE DOCUMENTO?", "SISTEMA", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No) Then
                Exit Sub
            Else
                Try
                    Connect_Database()
                    Command = New SqlCommand("SP_SILV_TYPE_DOCTOS_EDIT", connection) With {.CommandType = CommandType.StoredProcedure}
                    With Command
                        .Parameters.Add("@ID_FOLIOS_DOCTOS", SqlDbType.Int).Value = TXT_ID.Text
                        .Parameters.Add("@TY_NAME_TYPE_DOCTO", SqlDbType.NVarChar, 100).Value = TXT_NAME.Text
                        ...
                        .Parameters.Add("@TY_ACTIVE_INACTIVE", SqlDbType.Int).Value = C_ACTIVE_INACTIVE.CheckState
                        .Parameters.Add("@TY_USER_UPDATE", SqlDbType.NVarChar, 100).Value = frn_main_form.LBL_USERNAME.Caption
    ...
    Private Sub BTN_DELETE_ItemClick(sender As Object, e As ItemClickEventArgs) Handles BTN_DELETE.ItemClick
        ...
        If (VALIDA = True) Then
            ...
            Exit Sub
        Else
            If (XtraMessageBox.Show("¿DESEA ELIMINAR EL REGISTRO DEL TIPO DE DOCUMENTO?", "SISTEMA", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.No) Then
                Exit Sub
            Else

                Try
                    Connect_Database()
                    Command = New SqlCommand("SP_SILV_TYPE_DOCTOS_DELETE", connection) With {.CommandType = CommandType.StoredProcedure}
                    With Command
                        .Parameters.Add("@ID_FOLIOS_DOCTOS", SqlDbType.Int).Value = TXT_ID.Text
    ...
    Private Sub G_DATA_DoubleClick(sender As Object, e As EventArgs) Handles G_DATA.DoubleClick
        Try
            ...
            TXT_NAME.Text = CType(G_DATA.GetRowCellValue(G_DATA.FocusedRowHandle, "NOMBRE TIPO DOCUMENTO"), String)
    ...
    Private Sub frm_type_doctos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FILL_DATA()
    End Sub
